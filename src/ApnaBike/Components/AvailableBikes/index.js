...
export default function AvailableBikes(props) {
    ...
    let totalBill = 0;
    const { pickUpDate, dropoff, city } = useParams();
    var difference = Math.floor(((Date.parse(dropoff) - Date.parse(pickUpDate)) / 86400000) + 1);
    ...
    async function fetchAvailableBikes() {
        let bikes = await fetch(BaseUrl + `client/bikes/searchbikes?bookedFrom=${pickUpDate}&bookedTill=${dropoff}&city=${city}`)
        let availablebikes = await bikes.json();
        if (bikes.status === 200) {
            console.log('fetched bikes', availablebikes.Bikes);
        ...
        <>
            ...
            <div className="allBikes">
                <div className="container p-4">
                                    ...
                                    <div class="overlay " id={'detail' + bike.bike_id} >
                                        <div className="selectedDatesOrder p-4" key={bike.bike_id} >
                                            ...
                                            <div className="dateAndPrice">
                                                ...
                                                <h5>Extra Options :</h5>
                                                <div >
                                                    <div className="helmet" ><input type="number" placeholder="0" min="0" max="2" className="form-control" onChange={(e) => finalBill(e)} /> <strong>Helmet</strong></div>
                                                    <p>{helmet}</p>
                                                </div>
                    ...
                    {/* <Row className="p-0 " >
                        {history.location.state.booked.map(bike =>
                            <div className="col-lg-4 col-md-6 mb-3 p-3" key={bike.bike_id}>
                                <Card className="priceCard" id={bike.bike_id} key={bike.bike_id}>
                                    <Row>
                                        <img src={`./${bike.image}`} alt="bikeImg" />
                                    ...
                                    <div className="rentRow">
                                        <p>Booked Till </p>
                                        <strong>{bike.booked_till}</strong>
                                    </div>
                ...
                <Modal isOpen={signinModal} toggle={toggleSigninModal} className="loginModal p-2">
                    ...
                    <ModalBody>
                        <form onSubmit={(e) => handleSubmit(e)} className="pl-4 pr-4 mt-1">
                            <FormGroup>
                                <Input id="email" type="email" placeholder="Apnabike@gmail.com"></Input>
                            </FormGroup>
                            <FormGroup>
                        ...
                        <div className="signinFooter mt-5">Not a Member? <Link to="/signup"><strong> Create an Account </strong></Link></div>
                    </ModalBody>
                </Modal>
